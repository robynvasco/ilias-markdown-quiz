# MarkdownQuiz Plugin - API Key Encryption Security

## Overview

The MarkdownQuiz plugin now implements **AES-256-CBC encryption** for all API keys stored in the database. This addresses the critical security vulnerability of storing sensitive credentials in plain text.
...
**New Files:**
- `classes/platform/class.ilMarkdownQuizEncryption.php` - Encryption service class
- `test/test_encryption.php` - Unit tests for encryption

**Modified Files:**
- `classes/platform/class.ilMarkdownQuizConfig.php` - Added encryption/decryption
- `classes/class.ilMarkdownQuizConfigGUI.php` - Changed text inputs to password inputs
- `classes/class.ilMarkdownQuizPlugin.php` - Added migration on plugin update
...
Form Submission
    ↓
ilMarkdownQuizConfig::set($key, $value)
    ↓
Detect if value is API key
    ↓
ilMarkdownQuizEncryption::encrypt($value)
    ↓
  ...
  - Base64 encode
    ↓
Store encrypted value in xmdq_config table
```
...
Application needs API key
    ↓
ilMarkdownQuizConfig::get($key)
    ↓
Detect if key is encrypted (API key field)
    ↓
ilMarkdownQuizEncryption::decrypt($value)
    ↓
  - Base64 decode
...
2. **afterUpdate() Hook Called**
   - `ilMarkdownQuizPlugin::afterUpdate()` executes

3. **Migration Logic**
   - `ilMarkdownQuizEncryption::migrateApiKeys()` runs
...
#### Initial Setup
1. Navigate to Administration → Extending ILIAS → Plugins → MarkdownQuiz → Configure
...
```sql
SELECT name, value FROM xmdq_config WHERE name LIKE '%_api_key';
```
...
```php
// In class.ilMarkdownQuizConfig.php
...
```php
// In class.ilMarkdownQuizEncryption.php
...
)->withValue(ilMarkdownQuizConfig::get("new_service_api_key"))
  ->withAdditionalTransformation($this->refinery->custom()->transformation(
    function ($v) {
        ilMarkdownQuizConfig::set('new_service_api_key', $v);
    }
))->withRequired(true);
```
...
$encrypted = ilMarkdownQuizEncryption::encrypt($plainText);

// Decrypt a value
$decrypted = ilMarkdownQuizEncryption::decrypt($encrypted);

// Check if encrypted
$isEncrypted = ilMarkdownQuizEncryption::isEncrypted($someValue);
```
...
```php
require_once 'classes/platform/class.ilMarkdownQuizEncryption.php';
use platform\ilMarkdownQuizEncryption;
