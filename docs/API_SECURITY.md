...
## Overview
Implemented comprehensive API security measures for MarkdownQuiz plugin including:
- ✅ Certificate Pinning
...
### 1. Circuit Breaker (`class.ilMarkdownQuizCircuitBreaker.php`)
...
### 2. Response Validator (`class.ilMarkdownQuizResponseValidator.php`)
...
- Markdown quiz format validation (questions, options, correct answers)
...
- `validateMarkdownQuizFormat()` - Validate quiz markdown format
...
### 3. Request Signer (`class.ilMarkdownQuizRequestSigner.php`)
...
### 4. Certificate Pinner (`class.ilMarkdownQuizCertificatePinner.php`)
...
### OpenAI Class (`class.ilMarkdownQuizOpenAI.php`)
```php
public function generateQuiz() {
    try {
        ...
        $signature = ilMarkdownQuizRequestSigner::signRequest('openai', $payload, $api_key);
...
### Google AI Class (`class.ilMarkdownQuizGoogleAI.php`)
Same pattern as OpenAI, service name: `'google'`

### GWDG Class (`class.ilMarkdownQuizGWDG.php`)
...
✅ Markdown Validation: Validates question format (?, 4 options, 1 correct)
...
### Admin Tools
- Circuit breaker status: `ilMarkdownQuizCircuitBreaker::getStatus()`
- Reset circuit: `ilMarkdownQuizCircuitBreaker::reset('openai')`
- Reset all: `ilMarkdownQuizCircuitBreaker::resetAll()`
- Get certificate: `ilMarkdownQuizCertificatePinner::getCurrentFingerprint('api.openai.com')`

### Monitoring
Circuit breaker state stored in `$_SESSION['mdquiz_circuit_breaker']`:
```php
[
    'openai' => [
...
```
classes/security/
├── class.ilMarkdownQuizCircuitBreaker.php      (148 lines)
├── class.ilMarkdownQuizResponseValidator.php   (207 lines)
├── class.ilMarkdownQuizRequestSigner.php       (121 lines)
└── class.ilMarkdownQuizCertificatePinner.php   (158 lines)
