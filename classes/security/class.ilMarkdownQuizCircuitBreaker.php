...
{
    /** @var array<int, string> Allowed service identifiers */
    private const ALLOWED_SERVICES = ['openai', 'google', 'gwdg'];
     ...
     * 
     * @param string $service Service name
     * @throws ilMarkdownQuizException If circuit is open and timeout not expired
     */
     ...
     */
    private static function validateService(string $service): void
    {
        if (!in_array($service, self::ALLOWED_SERVICES, true)) {
            throw new ilMarkdownQuizException("Invalid service for circuit breaker: " . $service);
        }
    }
}
