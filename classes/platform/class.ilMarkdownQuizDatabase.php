...
/**
 * Datenbankzugriffsklasse für das MarkdownQuiz Plugin
 * 
 ...
 * Verwendung:
 * ```php
 * $db = new ilMarkdownQuizDatabase();
 * $db->insert('xmdq_config', ['name' => 'api_key', 'value' => 'encrypted_key']);
 * ```
 * 
 * @package platform
 */
class ilMarkdownQuizDatabase
{
    ...
    const ALLOWED_TABLES = [
        'xmdq_config',           // Plugin-Konfiguration (API-Keys, Einstellungen)
        'rep_robj_xmdq_data'     // Quiz-Inhalte und Metadaten
    ];
     ...
     * @return void
     * @throws ilMarkdownQuizException Bei ungültigem Tabellennamen oder DB-Fehler
     * 
     * @example
     * ```php
     * $db->insert('xmdq_config', [
     *     'name' => 'system_prompt',
     ...
     * $db->insertOnDuplicatedKey('xmdq_config', [
     *     'name' => 'ai_enabled',
     *     'value' => true
     * ]);
     * ```
     ...
     * $db->delete('xmdq_config', ['name' => 'old_api_key']);
     * ```
     */
     ...
     * $all = $db->select('xmdq_config');
     * 
     * // Spezifische Config abrufen
     * $config = $db->select('xmdq_config', ['name' => 'api_key']);
     * 
     * // Mit ORDER BY und LIMIT
     * $recent = $db->select('rep_robj_xmdq_data', null, ['id', 'title'], 'ORDER BY id DESC LIMIT 10');
     * ```
     */
     ...
     * @throws ilMarkdownQuizException Bei DB-Fehler
     * 
     * @example
     * ```php
     * $newId = $db->nextId('rep_robj_xmdq_data');
     * $db->insert('rep_robj_xmdq_data', ['id' => $newId, 'title' => 'New Quiz']);
     * ```
     */
