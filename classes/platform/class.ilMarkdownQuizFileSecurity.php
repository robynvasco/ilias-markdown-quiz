...
/**
 * File security helper for MarkdownQuiz plugin
...
class ilMarkdownQuizFileSecurity
{
    ...
    /**
     * Find first available executable path from candidates.
     *
    ...
    private static function findExecutable(array $candidates): ?string
    {
        ...
        $directories = explode(PATH_SEPARATOR, $path_env);
        foreach ($candidates as $candidate) {
            $binary = basename($candidate);
            foreach ($directories as $directory) {
                $full_path = rtrim($directory, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . $binary;
    ...
    public static function validateZipSafety(string $zip_path): void
    {
        ...
        if ($zip->open($zip_path) !== true) {
            throw new ilMarkdownQuizException("Failed to open ZIP file for validation");
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if ($stat === false) {
                $zip->close();
                throw new ilMarkdownQuizException("Failed to read ZIP file statistics");
            }
    ...
    {
        return self::findExecutable(['/usr/bin/clamdscan', '/usr/local/bin/clamdscan', '/opt/homebrew/bin/clamdscan']) !== null
            || self::findExecutable(['/usr/bin/clamscan', '/usr/local/bin/clamscan', '/opt/homebrew/bin/clamscan']) !== null;
    }
    ...
    public static function scanFileWithAntivirus(string $file_path): array
    {
        ...
        $scanner = self::findExecutable(['/usr/bin/clamdscan', '/usr/local/bin/clamdscan', '/opt/homebrew/bin/clamdscan']);
        if ($scanner === null) {
            $scanner = self::findExecutable(['/usr/bin/clamscan', '/usr/local/bin/clamscan', '/opt/homebrew/bin/clamscan']);
        }
        ...
        $command = escapeshellcmd($scanner) . ' --no-summary ' . escapeshellarg($file_path) . ' 2>&1';
        $output_lines = [];
        $exit_code = 0;
        exec($command, $output_lines, $exit_code);
        $output = implode("\n", $output_lines);
        $clean = $exit_code === 0 && strpos($output, 'FOUND') === false;
