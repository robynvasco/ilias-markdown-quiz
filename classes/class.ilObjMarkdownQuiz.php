...
class ilObjMarkdownQuiz extends ilObjectPlugin
{
    ...
    public function doRead(): void
    {
        ...
        $res = $DIC->database()->query("SELECT md_content, is_online, last_prompt, last_difficulty, last_question_count, last_context, last_file_ref_id FROM rep_robj_xmdq_data WHERE id = " .
            $DIC->database()->quote((int)$this->getId(), "integer"));
        while ($row = $DIC->database()->fetchAssoc($res)) {
            $this->md_content = (string) $row["md_content"];
            $this->online = (bool) ($row["is_online"] ?? 0);
            $this->last_prompt = (string) ($row["last_prompt"] ?? '');
            $this->last_difficulty = (string) ($row["last_difficulty"] ?? 'medium');
            $this->last_question_count = (int) ($row["last_question_count"] ?? 5);
            $this->last_context = (string) ($row["last_context"] ?? '');
            $this->last_file_ref_id = (int) ($row["last_file_ref_id"] ?? 0);
        }
    }
    ...
    public function doUpdate(): void
    {
        ...
        $DIC->database()->replace(
            ...
            [
                ...
                "is_online" => ["integer", (int)$this->online],
                "last_prompt" => ["text", $this->last_prompt],
                "last_difficulty" => ["text", $this->last_difficulty],
                "last_question_count" => ["integer", $this->last_question_count],
                "last_context" => ["text", $this->last_context],
                "last_file_ref_id" => ["integer", $this->last_file_ref_id]
            ]
        );
    }
